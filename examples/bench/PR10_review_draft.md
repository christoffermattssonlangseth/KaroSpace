...
pathway enrichment, tutorial overlay, feature modules, and region groups are all
...
1. **Test suite deleted with no replacement.** Commit `3651a53` removes
   `tests/test_analytics.py`, `tests/test_feature_sidecar_export.py`, and
...
2. **pseudobulk log2FC / p-value come from different models** (see inline on
   ...
   DESeq2's — a feature can be "significant" with a mismatched log2FC.
...
4. **Public API cleanup** (`export_to_html`: only current feature/annotation
   arguments remain) and the **sidecar binary format rename** (`feature`).
   This is an explicit breaking release.
...
> which can produce features flagged significant with a fold-change that disagrees
...
### karospace/exporter.py : 34475  (export_to_html signature)
> Public export arguments now use the current feature/annotation vocabulary
> only. Removed marker/cluster-DE arguments are no longer carried
> forward. The sidecar/`.karospace` packaging itself is retained, but the
> naming/manifest format uses feature terminology throughout. Note the breaking
> contract explicitly in the changelog.

### tests/  (deleted in commit 3651a53)
> This commit removes the entire test suite (test_analytics.py,
> test_feature_sidecar_export.py, test_pseudobulk_interactions.py) with no
...
> `_select_top_variable_features`, `_compute_morans_i`) — please restore those
> directly. For `test_feature_sidecar_export.py`, the `cluster_de` assertions are
